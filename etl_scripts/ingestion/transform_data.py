...
def transform_geolocation():
    geolocation_df = pd.read_csv(
        "./etl_scripts/ingestion/olist_geolocation_dataset.csv"
    )
    ...
    geolocation_df.to_csv(
        "./etl_scripts/ingestion/olist_geolocation_dataset_transformed.csv",
        sep=",",
        index=False,
        header=True,
    )
...
def transform_customer():
    ...
    customers_df.to_csv(
        "./etl_scripts/ingestion/olist_customers_dataset_transformed.csv",
        sep=",",
        index=False,
        header=True,
    )
...
def transform_orders_review():
    order_reviews = pd.read_csv(
        "./etl_scripts/ingestion/olist_order_reviews_dataset.csv"
    )
    ...
    order_reviews.drop_duplicates(
        subset=["order_id", "review_id"], keep="last", inplace=True
    )

    order_reviews.to_csv(
        "./etl_scripts/ingestion/olist_order_reviews_dataset_transformed.csv",
        sep=",",
        index=False,
        header=True,
    )
...
def transform_sellers():
    ...
    sellers_df["seller_zip_code_prefix"] = sellers_df["seller_zip_code_prefix"].astype(
        str
    )

    sellers_df.to_csv(
        "./etl_scripts/ingestion/olist_sellers_dataset_transformed.csv",
        sep=",",
        index=False,
        header=True,
    )
...
def transform_products():
    ...
    products_df.rename(
        columns={"product_name_lenght": "product_name_length"}, inplace=True
    )
    products_df.rename(
    ...
    products_df.to_csv(
        "./etl_scripts/ingestion/olist_products_dataset_transformed.csv",
        sep=",",
        index=False,
        header=True,
    )
